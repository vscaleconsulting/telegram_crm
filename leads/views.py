...
class ConversationView(generic.View):
    def get(self, *args, **kwargs):
        ...
        scroll = timezone.now().replace(tzinfo=None) + timedelta(hours=5, minutes=30) - messages[-1].datetime.replace(tzinfo=None) < timedelta(seconds=2) if messages else False

        print(timezone.now().replace(tzinfo=None) + timedelta(hours=5, minutes=30), messages[-1].datetime.replace(tzinfo=None), scroll)
